	...
	h.transformerPool = &sync.Pool{
		New: func() interface{} {
			...
			for i, repl := range h.Replacements {
				...
				if repl.re != nil {
          // This would require following changes in the upstream  icholy/replace/replace.go
          ...
          //    })
          //  }
					tr := replace.RegexpString(repl.re, finalReplace, h.ReplacePlaceholders)
          ...
          // }
          transforms[i] = replace.String(finalSearch, finalReplace, h.ReplacePlaceholders)
				}
			}
